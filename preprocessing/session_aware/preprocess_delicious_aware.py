...
DAYS_OFFSET = 0  # skip first 1/3 of data because it contains much fewer interactions (and users) in comparision with last 2/3

def prepare_time(data, time_key=TIME_KEY):
    print("prepare_time")
    """Assigns session ids to the events in data without grouping keys"""
    # timestamp = (dateutil.parser.parse(data[ITEM_KEY])).timestamp()
    # data[time_key] = data[time_key].apply(lambda x: (dateutil.parser.parse(x/1000)).timestamp())
    data[time_key] = data[time_key].apply(lambda x: int(x) /1000)
    return data
...
def load_data(file):
    data = pd.read_csv(file + '.dat', sep='\t',
                       names=[USER_KEY, "bookmarkID", ITEM_KEY, TIME_KEY])  # Items are tracks
